...
uint32_t ConnectorBase::toLittleEndian32(uint32_t val)
{
#ifdef IS_BIG_ENDIAN
...
void ConnectorBase::edidPutBlockCheckSum(uint8_t* edidBlock)
{
	...
	for (i = 0; i < XENDISPL_EDID_BLOCK_SIZE - 1; i++)
	{
...
void ConnectorBase::edidPutEssentials(edid* edidBlock)
{
	...
	uint16_t vendorId = (((('X' - '@') & 0x1f) << 10) |
						 ((('E' - '@') & 0x1f) <<  5) |
						 ((('N' - '@') & 0x1f) <<  0));
#ifdef IS_BIG_ENDIAN
	edidBlock->mfg_id[0] = vendorId & 0xff;
	edidBlock->mfg_id[1] = vendorId >> 8;
#else
	edidBlock->mfg_id[1] = vendorId >> 8;
	edidBlock->mfg_id[0] = vendorId & 0xff;
#endif
...
#ifdef IS_BIG_ENDIAN
	edidBlock->prod_code[0] = 'M';
	edidBlock->prod_code[1] = 'V';
#else
	edidBlock->prod_code[0] = 'M';
	edidBlock->prod_code[1] = 'V';
#endif
...
void ConnectorBase::edidPutStandardTiming(edid* edidBlock, int index,
										  uint32_t xres, uint32_t yres)
{
	std_timing* mode = &edidBlock->standard_timings[index];
	uint32_t aspect;
	...
	{
		throw Exception("Unsupported aspect ratio for this resolution",
						EINVAL);
	}

	/* X resolution: (datavalue + 31) * 8, range 256-2288 pixels. */
	if ((xres / 8) - 31 > 255)
	{
		throw Exception("Resolution too big for standard timings",
						EINVAL);
	}
	mode->hsize = (xres / 8) - 31;

	/*
	 * Image aspect ratio and vertical frequency (datavalue + 60,
	 * range 60-123 Hz).
	 */
	mode->vfreq_aspect = ((aspect << 6) | (EDID_REFRESH_RATE_HZ - 60));
}
										  ...
										  uint32_t dpi)
{
	...
	desc->pixel_clock = toLittleEndian32(clock / 10000);
...
size_t ConnectorBase::getEDID(grant_ref_t startDirectory, uint32_t size)
{
	...
	std::unique_ptr<XenGnttabBuffer> edidBuffer(
		new XenGnttabBuffer(mDomId, refs.data(), refs.size()));

	if (!edidBuffer)
	{
		throw Exception("Failed to create shared buffer for EDID", ENOMEM);
	}
